...
> 対象: Session 3-8-19（remote の枝から手元にブランチを作る）完了時点の実装
> 最終更新: 2026-08-30
...
### 14.18 Session 3-8-1 〜 3-8-19 の範囲外
...
| 一覧を開く前の fetch                               | しない。開くたびにネットワークへ出る形にしない ── 出るのは最後に取得した時点の写しで、そのことを面に書く（§14.27）                            |
| remote-tracking の掃除（`fetch --prune`）          | 置いていない。消えた枝が残って見えるのは fetch の領分で、一覧を開く操作に消す働きを混ぜない（§14.27）                                         |
