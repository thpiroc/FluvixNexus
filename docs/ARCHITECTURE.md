...
> 対象: Session 3-8-11（Git のコミット履歴）完了時点の実装
> 最終更新: 2026-08-25
...
### 14.18 Session 3-8-1 〜 3-8-11 の範囲外
...
| Graph（枝の線）/ stash / rebase / tag            | 履歴は 3-8-11 で**読むところまで**（§14.19）。線を引くには全部の親と並べ方が要り、他の3つは書き込みになる            |
...
| commit 1件の詳細 / その差分                      | 出さない。開くには rev を渡すことになり、差分で rev の欄を作らなかった線に触れる（§14.16・§14.19）                   |
| 履歴からの revert / cherry-pick / reset / amend  | 1つも置いていない。履歴の面から動く git は `log` の1本だけになる（§14.19）                                           |
| 完全な commit hash を Renderer へ渡す            | 渡さない。出すのは短い方だけで、指して何かを頼む先がそもそも無い（§14.19）                                           |
