...
> 対象: Session 3-8-16（remote の管理）完了時点の実装
> 最終更新: 2026-08-27
...
│       │   ├── useGitRepository.ts     状態の保持・いつ調べ直すか（`files:changed` / `git:changed` の合流）・Stage / Unstage / Commit / Push / ブランチ / 差分 / 破棄 / 履歴 の実行（§14.5・§14.11・§14.12・§14.14・§14.15・§14.16・§14.19）
...
Main domain       git/gitRepository.ts   噛み合わせ（読み取り）
   ...
   ├── git/gitOutput.ts        出力の読み取り / パスの比較 / ブランチの一覧 / 履歴（純粋・§14.14・§14.19）
...
### 14.18 Session 3-8-1 〜 3-8-16 の範囲外
...
| remote の URL を変える（`set-url`）              | 置いていない。黙って上書きすると、送り先が入れ替わったことに誰も気づかないまま Push が別のところへ飛ぶ（§14.24）      |
| remote の rename（`remote rename`）              | 置いていない。追跡先（`branch.*.remote`）まで書き換わる ── 消して足し直すのとは別の設計が要る（§14.24）               |
| Push / Pull の送り先を選ぶ                       | 選べない。要求は今も `void` で、決めるのはリポジトリの設定になる ── 増えたのは**登録簿を編集する口**だけ（§14.24）    |
| `git://` / `http://` / ローカルのパスの remote   | 登録できない。通す形は3つだけで、**知らない形は危なくなくても通さない**（§14.24）                                     |
| 認証情報を含む URL（`https://<token>@…`）        | 断る。通すと、アプリが利用者の token を `.git/config` へ平文で書くことになる（設計判断 7・§14.24）                    |
| remote の URL を Renderer へ渡す                 | 渡さない。一覧に載るのは**表示用のラベル**だけで、そこから URL は組み立て直せない（§14.24）                           |
...
| remote の一括削除 / `remote prune`               | 置いていない。渡せるのは1件で、複数を渡せる欄は作らない（Stage の「任意の複数」と同じ線。§14.24）                     |
...
`git remote add` は名前が既にあれば**失敗する。** それがここで欲しい振る舞いになる ── `set-url` は黙って上書きするため、既に remote を設定しているリポジトリで押すと、**送り先が入れ替わったことに誰も気づかないまま Push が別のところへ飛ぶ。**
