...
> 対象: Session 3-8-14（ブランチの削除 / rename）完了時点の実装
> 最終更新: 2026-08-27
...
#### 確認を挟むのは、Git ではこれが1つめ（Session 3-8-14 で2つになった）

Stage / Unstage も Commit も Push も、押しても失われるものが無い（あるいは失われるなら git が断る）。確認は**押した人にしか止められないもの**に限ってある（§12.6）。

3-8-9 の時点ではこれが唯一だったが、3-8-14 でブランチの削除が2つめになる（§14.22）。**2つの重さは同じではない。**
...
### 14.18 Session 3-8-1 〜 3-8-14 の範囲外
