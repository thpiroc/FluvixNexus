...
### Shortcuts S1〜S6（v1.0.0 の後：キーボードショートカット改善）

v1.0.0 の初期フィードバックを受けたキーボードショートカット改善。main を保護したまま作業ブランチで進めた。S1 と S2 は main から、S3 → S4 → S5 → S6 は順に積んで作り、**`feat/shortcuts-integration` で1本にまとめた**（main へは未 Merge。下の「S1〜S6 の統合」）。設計は ARCHITECTURE.md §13.6.2・§18.6・§18.9〜§18.12。

| 段  | ブランチ                              | 内容                                                                              | production build の確認 |
| --- | ------------------------------------- | --------------------------------------------------------------------------------- | ----------------------: |
| S1  | `feat/shortcuts-terminal-enter-paste` | Terminal の AI CLI モード（Enter / Shift+Enter / Ctrl+Enter / Ctrl+V の読み替え） |                   16/16 |
| S2  | `feat/shortcuts-builtin-rows`         | 組み込みの打鍵（Monaco・入力欄・xterm）を「変更不可」の行として一覧に並べる       |                     9/9 |
...
- S6 の時点では S1（AI CLI モード）と S2（組み込みの行）が積み上げに無かったので、この44項目には含まれていない。統合後に確かめ直した（次の節）

#### S1〜S6 の統合（`feat/shortcuts-integration`）

main（`9ca5857`）から作った統合ブランチに、`git merge --no-ff` で **S6 の tip（S3〜S5 を含む）→ S1 → S2** の順に入れ、統合の修正を別の commit にした。各 S の commit hash はそのまま残る（Notion の記録と対応する）。

| 段                   | commit    | 競合            |         `npm run verify` |
| -------------------- | --------- | --------------- | -----------------------: |
| S6（S3〜S5 を含む）  | `c0976e1` | なし            | 264 files / 4,754 passed |
| S1                   | `4fd9b0a` | なし            | 266 files / 4,774 passed |
| S2                   | `1a826ec` | 4ファイル（下） | 267 files / 4,801 passed |
| 統合の修正（コード） | `c256856` | ──              | 268 files / 4,828 passed |

**S2 の競合。** S2 は読むだけの一覧に組み込みの群を足し、S4 / S5 は同じ所を編集画面に書き換えていた（S3 の上では競合しない。S4 で初めて出る）。

| ファイル                                        | 解き方                                                                                                                                                                                        |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `settings/KeyboardShortcutsView.tsx`（5箇所）   | S5 の編集画面をそのまま取り、S2 の足した分（組み込みの群を Command の群の後ろへ・組み込みの行の部品・「該当なし」は両方0件のときだけ）を手で載せ直した                                        |
| `i18n/locales/ja.ts` / `en.ts`（各1箇所）       | 両方の文言を残し、統合後の画面と矛盾する2つを消した：S2 の `viewOnlyNote`（「この版では打鍵を変更できません」）と S4/S5 に残っていた `terminalNote`（「端末の文字の大きさはここに並ばない」） |
| `commands/commandLocalization.test.ts`（1箇所） | 翻訳キーの一覧を合わせ、上の2つを外した                                                                                                                                                       |

`settings/KeyboardShortcuts.dom.test.ts` は自動でマージされた。計画の段階では S2 のテスト（「一覧にボタンが0個」「この版では変更できない と出る」）が S4 と矛盾して落ちると見ていたが、S4 が同じ箇所を書き換えていたため S4 の側が残り、**テストの失敗は出なかった**。S2 の「組み込みの行は押せる要素を持たない」は残り、統合後の約束（組み込みの行に編集ボタンを出さない）を守るテストになっている。

**統合の修正（`c256856`）。**

- 組み込みの一覧に AI CLI モードの3行（改行 Enter・Shift+Enter／送信 Ctrl+Enter／貼り付け Ctrl+V）を「AI CLI モードのタブ」と断って足し、既存の「Ctrl+V をシェルへそのまま送る」を「通常のタブ」と断った（行は 15 → 18）
- **組み込みの表と予約キーの表は1つにまとめなかった。** 持つものが違い（表示名・群・効く場所 / 理由・条件）、片方から作ると表の形ごと変えることになるため。代わりに `keybindings/builtinReservedKeys.test.ts` で2方向を突き合わせた（ARCHITECTURE.md §18.11）
- そのテストが見つけた2つの抜けを予約キーの側で補った：一覧が見せている `Ctrl++`（`keybindings.json` に手で書ける）と、端末の Ctrl+Enter（`terminalSubmit`。通常のタブは xterm が CR として送り伝播を止め、AI CLI モードのタブは送信として取る ── どちらも割り当ては効かない）
- `terminalSubmit` を足したことで、S5 のテスト3件（`reservedKeys` / `keyWarnings` / `shortcutRows`）が「条件の無い command に Ctrl+Enter → 予約は `gitCommit` だけ」を期待して落ちた。条件の無い command は端末の中でも効く側なので、`gitCommit` と `terminalSubmit` の両方が正しい ── 期待を直した。直す途中で「エディターだけ」を `['editorFocused']` で書いた確認を1行足して落ちた（`when` はエディターと端末が同時に focus を持たないことを知らない）。このアプリのエディターだけの条件は `editorFocused` + `!terminalFocused`（既存の行）なので、足した行を消した

**production build の確認（80項目・全 PASS）。**

| スクリプト          | 項目 | 内容                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------- | ---: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S6 の確認をそのまま |   44 | ファイルなし・編集と再起動・壊れたファイル・予約キーの実害（Commit 欄 / F2 / 端末の Ctrl+- で両方が動く）・境界。S1 が `onAppKey` を変えた後も、端末の Ctrl+- は文字の大きさと割り当ての両方が動く                                                                                                                                                                                                        |
| S1 の確認をそのまま |   16 | 通常のタブで Enter / Shift+Enter が実行・受信バイト（通常 `0d 0d 0d 16`／AI CLI `0a 0a 0d` ＋ bracketed paste・二重送信なし）・OFF に戻す・タブ単位                                                                                                                                                                                                                                                       |
| 統合で足した確認    |   20 | 組み込みの群が Command の後ろ・行18・ボタンも `data-command` も無い・Command は40件で全行に編集ボタン・注記に矛盾が無い・AI CLI の行と効く場所・横スクロールなし・検索（組み込みだけ / AI CLI / 該当なし）・条件の無い command に Ctrl+Enter で Commit 欄と端末の両方の予告・**対照（Files では走る）に対して通常のタブと AI CLI モードのタブでは走らない**・すべて戻しても行18・英語・Renderer エラー0件 |

**統合で踏んだもの。**

- `git switch -c <name> origin/main` は **`origin/main` を upstream にする。** そのままだと `git pull` が main を取り込み、設定によっては push の宛先を取り違える。作った直後に `git branch --unset-upstream` した
- 競合の予測は `git merge-tree --write-tree --name-only <a> <b>` で、ブランチも作業ツリーも触らずに取れる（git 2.38 以降）。どの段で競合が生まれるかは、積み上げの途中の tip（S3 / S4 / S5）それぞれと試すと分かる
- 片側の変更が小さい競合（S2 の View）は、ファイルを `git checkout --ours` で大きい側に揃え、小さい側の差分（`git diff <base> <branch> -- <file>`）を手で載せ直すほうが、5つの hunk を1つずつ解くより取りこぼしが少ない
