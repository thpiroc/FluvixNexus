...
**配布物へそのまま写すパッケージ。** src が import せず `extraResources` で写すパッケージは、`tools/third-party-notices.mjs` の `BUNDLED_PACKAGES` から数え始め、`dependencies` を辿って全部載せる（載せ過ぎる側に倒す）。**今は該当するものが無い**（`BUNDLED_PACKAGES` は空）。feature/notion-mcp で Notion MCP サーバー（`@notionhq/notion-mcp-server` 2.5.1）を同梱していた間は 144件まで増えたが、アプリは MCP サーバーを同梱しない形に戻した（refactor/remove-legacy-notion-mcp。docs/ARCHITECTURE.md §21.9）。
...
`extraResources` は `LICENSE` と `THIRD_PARTY_NOTICES.txt` の2つだけ。feature/notion-mcp で足した Notion MCP サーバーの写し（`resources/mcp-servers/notion-mcp-server/`）は、旧 Notion MCP の撤去で外した（docs/ARCHITECTURE.md §21.9）。MCP サーバーは利用者が MCP Server Manager で登録し、その PC の上で動くもの（`npx`・`uvx`・`docker` など）を起動する。
