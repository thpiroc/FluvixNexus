 ...
 * ── adapter を立てるのも、`Source` の絶対パスを作るのも、`setBreakpoints` を
 * 組み立てるのも Main の責務で、Preload が提供するのは経路だけになる
 * （preload/api/index.ts）。
 *
 * `lsp` API が「渡す欄そのものが無い」ことで境界を作っているのと同じ形で、
 * こちらも渡せるのは**Workspace の中の相対位置と行**の2つだけになる。
 * adapter の名前も、DAP の method 名も、絶対パスも、渡す欄が存在しない
 ...
 *
 * Session 6-3 の時点で、**Debug Session を起動 / 停止する口はここに無い。**
 */
