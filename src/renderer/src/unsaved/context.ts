...
import type { LossAction, LossSource } from './types'

/**
 * 失われるものの確認を Renderer 全体へ配る仕組み。
 *
 * Provider は **App.tsx の一番外**に置く。中に置くと、
 * WorkspaceFolderProvider（Workspace を閉じる側）と EditorProvider / TerminalProvider
 * （失われるものを持つ側）の両方から参照できない（片方が必ず外になる）。
 *
...
export interface UnsavedChangesController {
   ...
   */
  readonly registerSource: (source: LossSource) => () => void
  /**
   * この操作を続けてよいか、必要なら利用者に尋ねる。
   *
   * 失われるものが1つも無ければ**尋ねずに true**。確認を出すこと自体が目的ではなく、
   * 失われるものがあるときだけ止めるのが目的なので、
   * 「毎回出るダイアログ」にはしない。
   */
  readonly confirmDiscard: (action: LossAction) => Promise<boolean>
}
