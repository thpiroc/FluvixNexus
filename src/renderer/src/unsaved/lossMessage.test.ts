...
describe('describeLossPrompt', () => {
  it('未保存のファイルだけなら、保存の話をする', () => {
    const prompt = describeLossPrompt('close-workspace', [unsavedFile('a.ts')], t)
  ...
  it('実行中のターミナルだけなら、終了の話をする', () => {
    const prompt = describeLossPrompt('close-window', [runningTerminal('terminal-1')], t)
  ...
  it('両方あるなら両方を言う', () => {
    const prompt = describeLossPrompt(
      'close-window',
      [unsavedFile('a.ts'), runningTerminal('terminal-1')],
      t
    )
  ...
  */
  it('救えるものが1つも無ければ保存を出さない', () => {
    const prompt = describeLossPrompt('close-window', [deletedFile('gone.ts')], t)
  ...
  it('1件でも救えるなら保存を出す', () => {
    const prompt = describeLossPrompt(
      'close-window',
      [deletedFile('gone.ts'), unsavedFile('a.ts')],
      t
    )
...
describe('describeLossNote', () => {
  it('ふつうの未保存には何も添えない', () => {
    expect(describeLossNote(unsavedFile('a.ts'), t)).toBeNull()
  })
  ...
  it('消えたファイルには理由と、救い出せる場所を添える', () => {
    /*
      ...
      （Session 4-2）。
    */
    expect(describeLossNote(deletedFile('gone.ts'), t)).toBe(
      'ディスク上から削除されています（Editor の「別名で保存」で救い出せます）'
    )
  })

  it('実行中のターミナルには、なぜ並んでいるかを添える', () => {
    expect(describeLossNote(runningTerminal('terminal-1'), t)).toBe('実行中のコマンドがあります')
  })
})
