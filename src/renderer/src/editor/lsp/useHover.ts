...
export function useHover(documents: EditorDocumentStore, workspaceId: string | null): void {
  ...
  useEffect(() => {
    ...
    const hoverModule = import('../monaco/lspHover')
    ...
    void hoverModule.then(({ registerLspHoverProvider }) => {
      if (disposed) {
        return
      }

      const registration = registerLspHoverProvider({
        documents,
