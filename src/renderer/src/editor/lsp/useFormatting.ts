...
export function useFormatting(documents: EditorDocumentStore, workspaceId: string | null): void {
  ...
  useEffect(() => {
    ...
    const formattingModule = import('../monaco/lspFormatting')
    ...
    void formattingModule.then(({ registerLspFormattingProvider }) => {
      if (disposed) {
        return
      }

      const registration = registerLspFormattingProvider({
        documents,
