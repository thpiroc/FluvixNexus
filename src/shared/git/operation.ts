...
export type GitOperationFailureReason =
   ...
   *
   * rename（Session 3-8-14）でも同じ理由で起こる ── 行き先の名前が
   * **別の既にあるブランチ**だった場合にあたる。付け替えないのは作成と同じで、
   * `--force`（`-M`）は「相手のブランチを消して名前を奪う」操作になる。
   *
   * **大文字小文字だけが違う名前は、ここへは来ない**（Session 3-8-14 で変わった）。
   * Windows では ref の実体が同じファイルになるため git は
   * 「既にある」と言うが、そのとき指されているのは**改名しようとしている
   * ブランチ自身**であって、別のブランチではない ── その1件は rename として
   * 通す（main/git/gitBranches.ts）。ここへ来るのは、完全に同じ綴りの ref が
   * 実在する場合だけになる。
   */
  | 'branch-exists'
  /**
   * 削除しようとしたブランチに、そこにしか無い commit がある（Session 3-8-14）。
   *
   * `git branch -d` の断り（`not fully merged`）にあたる。判断しているのは
   * git で、基準は「HEAD か、そのブランチの追跡先にマージ済みか」になる ──
   * アプリはその基準を持たず、一覧にマージ済みかどうかも載せていない
   * （shared/git/branch.ts）。**押す前には分からない**、数少ない結末の1つ。
   *
   * **`-D`（強制削除）の欄は作っていない。** 通せば commit がどこからも
   * 辿れなくなり、戻すには reflog から hash を探すことになる ── 押し間違いの
   * 代償が釣り合わない（`--force` push を持たないのと同じ線）。
   * 次の一手は「そのブランチをマージする」か、承知のうえで消すなら
   * Terminal パネルの `git branch -D` になる。
   */
  | 'branch-not-merged'
  /**
   * 削除しようとしたブランチが、今チェックアウトされている（Session 3-8-14）。
   *
   * 大半は「今そこに居るブランチの ✕ を押した」場合で、これは**読んだ状態
   * （`head`）だけで分かる**ため git を動かす前に返る（main/git/gitBranches.ts）。
   * 画面の側でもその行の ✕ は押せないようにしてあり、二重の備えになる。
   *
   * git 自身も同じ文言で断る（`used by worktree at ...`）── そちらは別の
   * worktree（§14.15 のとおり対象外の構成）で使われている場合にあたる。
   * 次の一手はどちらも同じ「別のブランチへ切り替える」なので、分類は1つにしてある。
   */
  | 'branch-checked-out'
  /**
  ...
  | 'commit-not-found'
  /**
   * 指した退避が、もうそこに無い（Session 3-8-15）。
   *
   * ## これだけは「消えた」だけでなく「ずれた」も含む
   *
   * `branch-not-found` / `commit-not-found` が指すのは**対象そのものが
   * 解けなくなった**ことだが、退避では**対象は在るのに別のものになっている**が
   * 普通に起こる ── `stash@{1}` は名前ではなく**上から数えた位置**で、
   * 退避を1つ作れば全部が1つずつ後ろへずれる（shared/git/stash.ts）。
   *
   * したがって Main は、押された瞬間に「その位置に居るのは、画面に出ていた
   * あの退避か」を hash で確かめ、違えば git を動かさずにここへ倒す
   * （main/git/gitStash.ts）。**確かめずに動かすと、押した人が見ていない
   * 退避が捨てられる**ことになり、それは戻せない。
   *
   * 次の一手はどちらの場合も同じ「一覧を開き直す」で、そこは分けなくてよい ──
...
export type GitPartialOperationStep =
  ...
  | 'github-repository'
  /**
   * 退避の中身は作業ツリーへ戻ったが、競合した（Session 3-8-15）。
   *
   * ## 3つめの `partly-applied` は、外ではなく手元で起こる
   *
   * 先の2つ（Commit & Push・公開）はどちらも「手元は変わったのに、**外へ出す**
   * ところで止まった」だった。こちらはネットワークが1度も出てこない ──
   * `git stash pop` は中身を作業ツリーへ**merge する**操作で、merge である以上
   * 競合しうる（3-8-9 が「競合しているファイルの差分」を対象外にしたのと
   * 同じ事情が、ここでは自分の操作から生まれる）。
   *
   * ## `failed` に丸めない理由も、前の2つと同じ形になる
   *
   * 丸めると利用者は「何も起きなかった」と読み、**もう一度 pop を押す。**
   * だが作業ツリーには既に競合の印が書き込まれている ── 押し直しても、
   * 今度は「作業ツリーの変更が上書きされる」として断られるだけになる。
   *
   * ## 退避は消えていない
   *
   * git は競合した pop で退避を**捨てない**（`The stash entry is kept in case
   * you need it again.`。実物で確かめてある）。ここでいちばん先に伝えるべきは
