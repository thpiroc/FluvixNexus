 ...
 *
 * ## 3-8-17 の rename も、行き先に同じこの関数を通す
 *
 * 追加・削除・URL の変更が指す側と、rename の**行き先**まで、remote 名は
 * すべてここを通る（main/ipc/handlers/git.ts）── 入口を分けると
 * 「元の名前としては通るが、新しい名前としては通らない」形が生まれる。
 * `git remote rename --end-of-options up2 -x` を git が受け取ってしまう
 * （実物で確かめてある）以上、行き先の側にも同じ穴が開いている。
 *
 * **「大文字小文字だけ違う名前」はここでは断らない。** それは1つの名前の
 * 形ではなく、2つの名前の**組み合わせ**で決まる ── 判断は git を動かす
 * 直前に置いてある（main/git/gitRemotes.ts の `applyGitRenameRemote`）。
 */
